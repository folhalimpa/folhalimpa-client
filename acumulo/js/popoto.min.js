popoto = function() {
    var a = {
        version: "1.1.2",
        start: function(b) {
            a.logger.info("Popoto " + a.version + " start on " + b);
            a.graph.mainLabel = b;
            if (void 0 === a.rest.CYPHER_URL) a.logger.error("popoto.rest.CYPHER_URL is not set but is required.");
            else {
                a.checkHtmlComponents();
                a.taxonomy.isActive && a.taxonomy.createTaxonomyPanel();
                if (a.graph.isActive)
                    if (a.graph.createGraphArea(), a.graph.createForceLayout(), "string" === typeof b || b instanceof String) {
                        var c = a.provider.getSchema(b);
                        void 0 !== c ? a.graph.addSchema(c) : a.graph.addRootNode(b)
                    } else a.graph.addSchema(b);
                a.queryviewer.isActive && a.queryviewer.createQueryArea();
                a.cypherviewer.isActive && a.cypherviewer.createQueryArea();
                a.update()
            }
        },
        checkHtmlComponents: function() {
            var b = d3.select("#" + a.graph.containerId),
                ...
                f = d3.select("#" + a.cypherviewer.containerId),
                e = d3.select("#" + a.result.containerId);
            b.empty() ? (a.logger.debug("The page doesn't contain a container with ID = \"" + a.graph.containerId + '" no graph area will be generated. This ID is defined in popoto.graph.containerId property.'),
                a.graph.isActive = !1) : a.graph.isActive = !0;
            ...
            f.empty() ? (a.logger.debug("The page doesn't contain a container with ID = \"" + a.cypherviewer.containerId + '" no cypher query viewer will be generated. This ID is defined in popoto.cypherviewer.containerId property.'), a.cypherviewer.isActive = !1) : a.cypherviewer.isActive = !0;
            e.empty() ? (a.logger.debug("The page doesn't contain a container with ID = \"" + a.result.containerId + '" no result area will be generated. This ID is defined in popoto.result.containerId property.'), a.result.isActive = !1) : a.result.isActive = !0
        },
        update: function() {
            a.updateGraph();
            var b = a.graph.getRootNode();
            b && void 0 !== b.label && (a.queryviewer.isActive && a.queryviewer.updateQuery(), a.cypherviewer.isActive && a.cypherviewer.updateQuery(), (a.result.isActive || 0 < a.result.resultListeners.length || 0 < a.result.resultCountListeners.length || 0 < a.result.graphResultListeners.length) && a.result.updateResults())
        },
    ...
    a.taxonomy.createTaxonomyPanel = function() {
        var b = d3.select("#" + a.taxonomy.containerId).append("ul").attr("class", "ppt-taxo-ul"),
        ...
        b.append("span").attr("class", "ppt-icon ppt-taxo-tag").html("&nbsp;");
        //window.alert(b.append("span").attr("class", "ppt-icon ppt-taxo-tag").html("&nbsp;"));    
        b.append("span").attr("class",
            "ppt-label").text(function(b) {
            //if (a.provider.getTaxonomyTextValue(b.label) == "Servidor")
        ...
        a.graph.DISABLE_COUNT || a.taxonomy.updateCount(d)
    };
        ...
        (function(b) {
            ...
            a.rest.post({
                ...
                for (var c = 0; c < b.length; c++) {
                    var d = 0;
                    a && a.hasOwnProperty("results") && 0 < a.results.length && (d = a.results[c].data[0].row[0]);
                    d3.select("#" + b[c].id).select(".ppt-count").text(" (" + d + ")")
                }
            }).fail(function(b, c, d) {
    ...
    a.taxonomy.addTaxonomyChildren = function(b) {
        b.each(function(b) {
            var c = d3.select(this),
                f = b.children;
            b.children && (b = c.append("ul").attr("class", "ppt-taxo-sub-ul").selectAll("li").data(f).enter().append("li").attr("id", function(a) {
                    return a.id
                }).attr("value", function(a) {
                    return a.label
                }), b.append("span").attr("class", "ppt-icon ppt-taxo-tag").html("&nbsp;"), b.append("span").attr("class", "ppt-label").text(function(b) {
                    return a.provider.getTaxonomyTextValue(b.label)
                }),
    ...
    a.taxonomy.onClick = function() {
        ...
        a.update();
        a.graph.mainLabel = b;
        void 0 !== a.provider.getSchema(b) ? a.graph.addSchema(a.provider.getSchema(b)) : a.graph.addRootNode(b);
    ...
    a.taxonomy.addChildrenData = function(b, c) {
        ...
        a.provider.getProvider(b.label).children.forEach(function(d) {
            var f =
                a.provider.getProvider(d),
                e = {
                    label: d,
                    id: "popoto-lbl-" + c++
                };
            f.hasOwnProperty("children") && (c = a.taxonomy.addChildrenData(e, c));
            a.provider.getProperty(d, "isSearchable") && b.children.push(e)
        });
        return c
    };
    a.tools = {};
    ...
    a.tools.SAVE_GRAPH = !1;
    a.tools.TOGGLE_TAXONOMY = !0;
    a.tools.TOGGLE_FULL_SCREEN = !0;

    a.tools.resetServidor = function() {
        for (; 0 < a.graph.force.nodes().length;) a.graph.force.nodes().pop();
        for (; 0 < a.graph.force.links().length;) a.graph.force.links().pop();
        a.graph.node.internalLabels = {};
        a.update();
        a.graph.mainLabel = "Servidor";
        //window.alert(a.graph.mainLabel)
        "string" === typeof a.graph.mainLabel || a.graph.mainLabel instanceof String ? void 0 !== a.provider.getSchema(a.graph.mainLabel) ? a.graph.addSchema(a.provider.getSchema(a.graph.mainLabel)) : a.graph.addRootNode(a.graph.mainLabel) : a.graph.addSchema(a.graph.mainLabel);
        a.graph.hasGraphChanged = !0;
        a.result.hasChanged = !0;
        a.update();
        a.tools.center()
    ...
    a.tools.center = function() {
        ...
        a.graph.svg.transition().attr("transform", "translate(" + a.graph.zoom.translate() + ") scale(" +
            a.graph.zoom.scale() + ")")
    };
    a.tools.toggleTaxonomy = function() {
        var b = d3.select("#" + a.taxonomy.containerId);
    ...
    a.tools.toggleFullScreen = function() {
        ...
        document.fullscreenElement || document.mozFullScreenElement || document.webkitFullscreenElement || document.msFullscreenElement ? document.exitFullscreen ? document.exitFullscreen() : document.msExitFullscreen ? document.msExitFullscreen() :
            document.mozCancelFullScreen ? document.mozCancelFullScreen() : document.webkitExitFullscreen && document.webkitExitFullscreen() : b.requestFullscreen ? b.requestFullscreen() : b.msRequestFullscreen ? b.msRequestFullscreen() : b.mozRequestFullScreen ? b.mozRequestFullScreen() : b.webkitRequestFullscreen && b.webkitRequestFullscreen(Element.ALLOW_KEYBOARD_INPUT)
    };
    a.graph = {};
    ...
    a.graph.TOOL_TAXONOMY =
        "Show/hide taxonomy panel";
    a.graph.TOOL_CENTER = "Centralizar";
    a.graph.TOOL_FULL_SCREEN = "Tela cheia";
    a.graph.TOOL_RESET = "Reiniciar";
    a.graph.TOOL_SAVE = "Save graph";
    ...
    a.graph.saveListeners = [];
    a.graph.onSave = function(b) {
        a.graph.saveListeners.push(b)
    };
    a.graph.setDefaultGraph = function(b) {
        a.graph.mainLabel = b
    };
    a.graph.createGraphArea = function() {
        var b = d3.select("#" + a.graph.containerId),
            c = b.append("div").attr("class", "ppt-toolbar");
        if (a.tools.RESET_GRAPH) c.append("span").attr("id", "popoto-reset-menu").attr("class", "ppt-icon ppt-menu reset").attr("title", a.graph.TOOL_RESET).on("click", a.tools.resetServidor);
        if (a.taxonomy.isActive && a.tools.TOGGLE_TAXONOMY) c.append("span").attr("id", "popoto-taxonomy-menu").attr("class", "ppt-icon ppt-menu taxonomy").attr("title", a.graph.TOOL_TAXONOMY).on("click", a.tools.toggleTaxonomy);
        if (a.tools.CENTER_GRAPH) c.append("span").attr("id", "popoto-center-menu").attr("class",
            "ppt-icon ppt-menu center").attr("title", a.graph.TOOL_CENTER).on("click", a.tools.center);
        ...
        if (a.tools.SAVE_GRAPH) c.append("span").attr("id", "popoto-save-menu").attr("class", "ppt-icon ppt-menu save").attr("title", a.graph.TOOL_SAVE).on("click", function() {
            if (0 < a.graph.saveListeners.length) {
                var b = a.graph.getSchema();
                a.graph.saveListeners.forEach(function(a) {
                    a(b)
                })
            }
        });
        b = b.append("svg").call(a.graph.zoom.on("zoom", a.graph.rescale));
    ...
    a.graph.centerRootNode = function() {
        a.graph.getRootNode().px =
            a.graph.getSVGWidth() / 2;
        a.graph.getRootNode().py = a.graph.getSVGHeight() / 2;
        a.update()
    };
    a.graph.getSVGWidth = function() {
    ...
    a.graph.rescale = function() {
        a.graph.svg.attr("transform", "translate(" + d3.event.translate + ") scale(" + d3.event.scale + ")")
    };
    ...
    a.graph.nodeExpandRelationshipListeners = [];
    a.graph.createForceLayout = function() {
        a.graph.force = d3.layout.force().size([a.graph.getSVGWidth(), a.graph.getSVGHeight()]).linkDistance(function(b) {
            return b.type === a.graph.link.LinkTypes.RELATION ?
                3 * a.graph.LINK_DISTANCE / 2 : a.graph.LINK_DISTANCE
        }).linkStrength(function(b) {
            return b.linkStrength ? b.linkStrength : a.graph.LINK_STRENGTH
    ...
    a.graph.on = function(b, c) {
        b === a.graph.Events.NODE_ROOT_ADD &&
            a.graph.rootNodeAddListeners.push(c);
        b === a.graph.Events.NODE_EXPAND_RELATIONSHIP && a.graph.nodeExpandRelationshipListeners.push(c);
        b === a.graph.Events.GRAPH_SAVE && a.graph.saveListeners.push(c)
    };
    a.graph.addRootNode = function(b) {
        0 < a.graph.force.nodes().length && a.logger.warn("popoto.graph.addRootNode is called but the graph is not empty.");
        ...
        a.provider.getIsAutoExpandRelations(b) && a.graph.node.expandRelationData(a.graph.getRootNode())
    };
    a.graph.addSchema = function(b) {
        0 < a.graph.force.nodes().length && a.logger.warn("popoto.graph.addSchema is called but the graph is not empty.");
        var c = 0;
        b.hasOwnProperty("rel") && (c = b.rel.length);
        c = {
            id: "0",
            type: a.graph.node.NodeTypes.ROOT,
            x: a.graph.getSVGWidth() / 2,
            ...
            label: b.label,
            fixed: !0,
            internalLabel: a.graph.node.generateInternalLabel(b.label),
            linkExpanded: !0,
            linkCount: c,
            count: 1
        };
        b.hasOwnProperty("value") && (c.value = {
        ...
        if (b.hasOwnProperty("rel"))
            for (var d = 0; d < b.rel.length; d++) a.graph.addSchemaRelation(b.rel[d], c, d + 1, b.rel.length)
    };
    a.graph.addSchemaRelation = function(b, c, d, f) {
        d = a.graph.addSchemaNode(b.node, c, d, f);
    ...
    a.graph.addSchemaNode = function(b, c, d, f) {
        var e = a.provider.getIsGroup(b),
            g = 0;
        b.hasOwnProperty("rel") && (g = b.rel.length);
        var h = a.graph.computeParentAngle(c);
        f = h ? 360 / (f + 1) * d : 360 / f * d;
        d = c.x + 200 * Math.cos(Math.PI / 180 * f - h);
        h = c.y + 200 * Math.sin(Math.PI / 180 * f - h);
        c = {
            id: "" + ++a.graph.node.idgen,
            parent: c,
            type: e ? a.graph.node.NodeTypes.GROUP : a.graph.node.NodeTypes.CHOOSE,
            label: b.label,
            fixed: !1,
            internalLabel: a.graph.node.generateInternalLabel(b.label),
            x: d,
        ...
        a.graph.force.nodes().push(c);
        if (b.hasOwnProperty("rel"))
            for (e = 0; e < b.rel.length; e++) a.graph.addSchemaRelation(b.rel[e], c, e + 1, b.rel.length);
        return c
    };
    a.graph.getRootNode = function() {
        return a.graph.force.nodes()[0]
    };
    a.graph.getSchema = function() {
        var b = {},
            c = a.graph.getRootNode();
        b[c.id] = {
            label: c.label
        };
        c.hasOwnProperty("value") && (b[c.id].value = c.value.attributes);
        var d = a.graph.force.links();
        0 < d.length && d.forEach(function(a) {
            var c = a.source,
                d = a.target;
            b.hasOwnProperty(c.id) || (b[c.id] = {
                label: c.label
            }, c.hasOwnProperty("value") && (b[c.id].value = c.value.attributes));
            b.hasOwnProperty(d.id) || (b[d.id] = {
                label: d.label
            }, d.hasOwnProperty("value") && (b[d.id].value = d.value.attributes));
            b[c.id].hasOwnProperty("rel") || (b[c.id].rel = []);
            b[c.id].rel.push({
                label: a.label,
                node: b[d.id]
            })
    ...
    a.graph.tick = function() {
        var b = a.graph.svg.selectAll("#" + a.graph.link.gID + " > g");
        b.selectAll("path").attr("d",
            function(b) {
                var c = a.graph.computeParentAngle(b.target),
                    f = b.target.x + a.graph.link.RADIUS * Math.cos(c),
                    e = b.target.y - a.graph.link.RADIUS * Math.sin(c),
                    g = b.source.x - a.graph.link.RADIUS * Math.cos(c),
                    c = b.source.y + a.graph.link.RADIUS * Math.sin(c);
                return b.source.x <= b.target.x ? "M" + g + " " + c + "L" + f + " " + e : "M" + f + " " + e + "L" + g + " " + c
            });
        b.selectAll(".ppt-textPath").attr("xlink:href", function(a) {
    ...
    a.graph.link.updateData = function() {
        a.graph.link.svgLinkElements = a.graph.link.svgLinkElements.data(a.graph.force.links(),
            function(a) {
                return a.id
            })
    };
    a.graph.link.removeElements = function() {
    ...
    a.graph.link.updateElements = function() {
        a.graph.link.svgLinkElements.attr("id",
            function(a) {
                return "ppt-glink_" + a.id
            });
        ...
        a.graph.link.svgLinkElements.selectAll("text").attr("id", function(a) {
            ...
            return b.type === a.graph.link.LinkTypes.VALUE ?
                "ppt-link-text-value" : 0 == b.target.count ? "ppt-link-text-relation disabled" : void 0 !== b.target.value ? "ppt-link-text-relation value" : "ppt-link-text-relation"
    ...
    a.graph.node.BACK_CIRCLE_R =
        70;
    a.graph.node.NODE_MAX_CHARS = 45;
    a.graph.node.PAGE_SIZE = 10;
    a.graph.DISABLE_COUNT = !1;
    ...
    a.graph.node.updateData = function() {
        ...
        a.graph.hasGraphChanged && !a.graph.DISABLE_COUNT && a.graph.node.updateCount();
        a.graph.hasGraphChanged = !1
    };
    a.graph.node.updateCount = function() {
        var b = [],
            c = a.graph.force.nodes().filter(function(b) {
                return a.graph.node.NodeTypes.VALUE !== b.type && a.graph.node.NodeTypes.GROUP !== b.type
            });
        c.forEach(function(c) {
            c = a.query.generateNodeCountQuery(c);
            b.push({
        ...
        a.rest.post({
            ...
            if (b) {
                b.hasOwnProperty("errors") && 0 < b.errors.length && a.logger.error("Cypher query error:" + JSON.stringify(b.errors));
                if (b.hasOwnProperty("results") &&
                    0 < b.results.length)
                    for (var d = 0; d < c.length; d++) c[d].count = b.results[d].data[0].row[0];
                else c.forEach(function(a) {
                    a.count = 0
                });
                0 < a.result.resultCountListeners.length && a.result.updateResultsCount();
                a.graph.node.updateElements();
                a.graph.link.updateElements()
            }
        }).fail(function(b, f, e) {
            a.logger.error(f + ': error while accessing Neo4j server on URL:"' + a.rest.CYPHER_URL + '" defined in "popoto.rest.CYPHER_URL" property: ' + e);
    ...
    a.graph.node.addBackgroundElements = function(b) {
        b.append("g").attr("class", "ppt-g-node-background").append("circle").attr("class", function(b) {
            ...
            void 0 !== b.value ? c += " selected-value" : b.type === a.graph.node.NodeTypes.ROOT ? c += " root" : b.type === a.graph.node.NodeTypes.CHOOSE ? c += " choose" : b.type === a.graph.node.NodeTypes.VALUE ? c += " value" : b.type === a.graph.node.NodeTypes.GROUP && (c += " group");
            return c
    ...
    a.graph.node.addForegroundElements = function(b) {
        ...
        c.on("mouseover", function() {
            d3.select(this).select(".ppt-rel-plus-background").transition().style("fill-opacity",
                .5)
        ...
        c.append("path").attr("class",
            "ppt-rel-minus-path").attr("d", "M 40,-45 25,-45 25,-40 40,-40 z");
        c.on("mouseover", function() {
        ...
        d.append("path").attr("class", "ppt-arrow").attr("d", "m -44.905361,-23 6.742,-6.742 c 0.81,-0.809 0.81,-2.135 0,-2.944 l -0.737,-0.737 c -0.81,-0.811 -2.135,-0.811 -2.945,0 l -8.835,8.835 c -0.435,0.434 -0.628,1.017 -0.597,1.589 -0.031,0.571 0.162,1.154 0.597,1.588 l 8.835,8.834 c 0.81,0.811 2.135,0.811 2.945,0 l 0.737,-0.737 c 0.81,-0.808 0.81,-2.134 0,-2.943 l -6.742,-6.743 z");
        d.on("click", function(b) {
            d3.event.stopPropagation();
            1 < b.page && (b.page--, a.graph.node.collapseNode(b), a.graph.node.expandNode(b))
        });
        c = c.append("g");
        c.append("circle").attr("class", "ppt-rarrow").attr("cx", "43").attr("cy", "-23").attr("r", "17");
        ...
        a.graph.DISABLE_COUNT || (b = b.filter(function(b) {
            return b.type !== a.graph.node.NodeTypes.GROUP
        }), b.append("rect").attr("x", a.graph.node.CountBox.x).attr("y", a.graph.node.CountBox.y).attr("width", a.graph.node.CountBox.w).attr("height", a.graph.node.CountBox.h).attr("class", "ppt-count-box"), b.append("text").attr("x", 42).attr("y", 48).attr("text-anchor",
            "middle").attr("class", "ppt-count-text"))
    };
    ...
    a.graph.node.updateBackgroundElements = function() {
        a.graph.node.svgNodeElements.selectAll(".ppt-g-node-background").selectAll(".ppt-node-background-circle").attr("class", function(b) {
            var c = "ppt-node-background-circle";
            b.type === a.graph.node.NodeTypes.VALUE ? c += " value" : b.type === a.graph.node.NodeTypes.GROUP ? c += " group" : void 0 !== b.value ? b.type === a.graph.node.NodeTypes.ROOT ? c += " selected-root-value" : b.type === a.graph.node.NodeTypes.CHOOSE && (c += " selected-value") : 0 == b.count ? c += " disabled" : b.type === a.graph.node.NodeTypes.ROOT ?
                c += " root" : b.type === a.graph.node.NodeTypes.CHOOSE && (c += " choose");
            return c
    ...
    a.graph.node.updateMiddlegroundElements = function() {
        ...
        b.filter(function(b) {
            ...
            if (b.type === a.graph.node.NodeTypes.ROOT) return b.value ?
                "ppt-node-ellipse selected-root-value" : 0 == b.count ? "ppt-node-ellipse root disabled" : "ppt-node-ellipse root";
            if (b.type === a.graph.node.NodeTypes.CHOOSE) return b.value ? "ppt-node-ellipse selected-value" : 0 == b.count ? "ppt-node-ellipse choose disabled" : "ppt-node-ellipse choose";
    ...
    a.graph.node.updateForegroundElements = function() {
        ...
        a.graph.DISABLE_COUNT || b.selectAll(".ppt-count-text").text(function(a) {
        ...
        b.selectAll(".ppt-rel-minus-icon").classed("disabled", function(a) {
            return !a.linkExpanded || 0 ==
                a.count || 0 == a.linkCount
        })
    };
    ...
    a.graph.node.chooseNodeClick = function(b) {
        ...
        if (!a.graph.node.chooseWaiting && !b.immutable) {
            a.graph.force.nodes().forEach(function(b) {
                b.type != a.graph.node.NodeTypes.ROOT && b.type != a.graph.node.NodeTypes.CHOOSE || !b.valueExpanded ||
                    a.graph.node.collapseNode(b)
            });
            a.graph.node.chooseWaiting = !0;
            a.logger.info("Values (" + b.label + ") ==> ");
            var c = a.query.generateNodeValueQuery(b);
            a.rest.post({
                statements: [{
                    statement: c.statement,
                    parameters: c.parameters
                ...
                c && c.hasOwnProperty("errors") && 0 < c.errors.length && a.logger.error(JSON.stringify(c.errors));
                ...
                a.graph.node.chooseWaiting = !1;
                a.logger.error(c + ': error while accessing Neo4j server on URL:"' + a.rest.CYPHER_URL + '" defined in "popoto.rest.CYPHER_URL" property: ' + e)
            })
        }
    };
    a.graph.node.parseResultData = function(a) {
        var b = [];
        if (a && a.hasOwnProperty("results") && 0 < a.results.length)
            for (var d = 0; d < a.results[0].data.length; d++) {
                for (var f = {}, e = 0; e < a.results[0].columns.length; e++) f[a.results[0].columns[e]] = a.results[0].data[d].row[e];
                b.push(f)
            }
        return b
    };
    a.graph.computeParentAngle = function(a) {
        ...
        if (a.parent) {
            var b = a.parent.x,
                ...
                f = a.x;
            a = a.y;
            var e = 100 / (Math.sqrt(Math.pow(b - f, 2) + Math.pow(d - a, 2)) - 100),
                b = ((f + e * b) / (1 + e) - f) / 100; - 1 > b && (b = -1);
            1 < b && (b = 1);
            b = Math.acos(b);
            d > a && (b = 2 * Math.PI - b)
        }
    ...
    a.graph.node.expandNode = function(b) {
        var c = b.page * a.graph.node.PAGE_SIZE,
            ...
            f = a.graph.computeParentAngle(b),
            e = 1;
        d.forEach(function(c) {
            var g;
            g = b.parent ? 360 / (d.length + 1) * e : 360 / d.length * e;
            var k = b.x + 100 * Math.cos(Math.PI / 180 * g - f);
            g = b.y + 100 * Math.sin(Math.PI / 180 * g - f);
            c = {
                id: ++a.graph.node.idgen,
                parent: b,
                attributes: c,
    ...
    a.graph.node.expandRelationship = function() {
        d3.event.preventDefault();
        a.graph.nodeExpandRelationshipListeners.forEach(function(a) {
            a(this)
        });
        var b = d3.select(this).data()[0];
        b.linkExpanded || a.graph.node.linkWaiting || b.valueExpanded || a.graph.node.expandRelationData(b)
    };
    a.graph.node.expandRelationData = function(b) {
        a.graph.node.linkWaiting = !0;
        a.logger.info("Relations (" + b.label + ") ==> ");
        var c = a.query.generateNodeRelationQuery(b);
        a.rest.post({
            statements: [{
                statement: c.statement,
                parameters: c.parameters
            ...
            c && c.hasOwnProperty("errors") && 0 < c.errors.length && a.logger.error(JSON.stringify(c.errors));
            c = a.graph.node.parseResultData(c);
            c = c.filter(function(b) {
                return a.query.filterRelation(b)
            });
            0 < c.length && a.graph.addRelationshipData(b, c);
            b.linkCount = c.length;
            b.linkExpanded = !0;
            a.graph.hasGraphChanged = !0;
    ...
    a.graph.addRelationshipData = function(b, c) {
        var d = a.graph.computeParentAngle(b),
            f = 1;
        c.forEach(function(e) {
            var g;
            g = d ? 360 / (c.length + 1) * f : 360 / c.length * f;
            var h = b.x + 100 * Math.cos(Math.PI / 180 * g - d);
            g = b.y + 100 * Math.sin(Math.PI / 180 * g - d);
            var k = a.provider.getIsGroup(e),
                h = {
                    id: "" + ++a.graph.node.idgen,
                    parent: b,
                    ...
                    label: e.label,
                    fixed: !1,
                    internalLabel: a.graph.node.generateInternalLabel(e.label),
                    x: h,
                    y: g
                };
            ...
            a.provider.getIsAutoExpandRelations(h.label) && void 0 == a.provider.getSchema(a.graph.mainLabel) && a.graph.node.expandRelationData(h);
            f++
        })
    };
    a.graph.node.collapseRelationship = function() {
        ...
        if (b.linkExpanded && 0 < b.linkCount && !a.graph.node.linkWaiting && !b.valueExpanded) {
            ...
            var c = a.graph.force.links().filter(function(c) {
                return c.source ===
                    b && c.type === a.graph.link.LinkTypes.RELATION
            });
            c.forEach(function(b) {
    ...
    a.queryviewer.QUERY_STARTER = "Estou procurando por";
    a.queryviewer.CHOOSE_LABEL = "escolha";
    ...
    a.queryviewer.updateQuery =
        function() {
            ...
            a.queryviewer.queryConstraintSpanElements.attr("id",
                function(a) {
                    return a.id
                }).attr("class", function(b) {
                return b.isLink ? "ppt-span-link" : b.type === a.graph.node.NodeTypes.ROOT ? "ppt-span-root" : b.type === a.graph.node.NodeTypes.CHOOSE ? b.ref.value ? "ppt-span-value" : "ppt-span-choose" : b.type === a.graph.node.NodeTypes.VALUE ? "ppt-span-value" : b.type === a.graph.node.NodeTypes.GROUP ? "ppt-span-group" : "ppt-span"
            }).text(function(a) {
                return a.term + " "
            });
    ...
    a.queryviewer.generateConstraintData = function(b, c) {
        ...
        b.forEach(function(b) {
            var c = b.source,
                e = b.target;
            b.type === a.graph.link.LinkTypes.RELATION && e.type !== a.graph.node.NodeTypes.GROUP && e.value && (c.type === a.graph.node.NodeTypes.GROUP && d.push({
                id: f++,
                type: c.type,
                ...
                ref: b
            }), e.type !== a.graph.node.NodeTypes.GROUP && (e.value ? d.push({
                id: f++,
                type: e.type,
                term: a.provider.getSemanticValue(e),
                ref: e
            }) : d.push({
                id: f++,
                type: e.type,
                term: "<" + a.queryviewer.CHOOSE_LABEL +
                    " " + a.provider.getSemanticValue(e) + ">",
                ref: e
            })))
        });
        return d
    ...
    a.queryviewer.generateData = function(b, c) {
        ...
        b.forEach(function(b) {
            var c = b.source,
                g = b.target;
            g.type === a.graph.node.NodeTypes.GROUP && f.push({
                id: e++,
                type: g.type,
    ...
    a.queryviewer.mouseOutSpan = function() {
        ...
        var b = d3.select(this).data()[0];
        if (b.ref) {
            var c = a.graph.svg.selectAll("#" +
                a.graph.link.gID + " > g").filter(function(a) {
                return a === b.ref
            });
    ...
    a.cypherviewer = {};
    a.cypherviewer.containerId =
        "popoto-cypher";
    a.cypherviewer.MATCH = "MATCH";
    a.cypherviewer.RETURN = "RETURN";
    a.cypherviewer.QueryElementTypes = Object.freeze({
        KEYWORD: 0,
        NODE: 1,
        SEPARATOR: 2,
        SOURCE: 3,
    ...
    a.cypherviewer.updateQuery = function() {
        ...
        a.cypherviewer.querySpanElements = a.cypherviewer.querySpanElements.data(a.cypherviewer.generateData(a.graph.force.links(), a.graph.force.nodes()));
        ...
        a.cypherviewer.querySpanElements.filter(function(b) {
            return b.type ===
                a.cypherviewer.QueryElementTypes.KEYWORD
        }).attr("class", "ppt-span").text(function(a) {
            return " " + a.value + " "
        });
        a.cypherviewer.querySpanElements.filter(function(b) {
            return b.type === a.cypherviewer.QueryElementTypes.SEPARATOR
        }).attr("class", "ppt-span").text(function(a) {
            return a.value + " "
        });
        a.cypherviewer.querySpanElements.filter(function(b) {
        ...
        a.cypherviewer.querySpanElements.filter(function(b) {
            ...
            return b.node === a.graph.getRootNode() ? b.node.value ?
                "ppt-span-root-value" : "ppt-span-root" : b.node.value ? "ppt-span-value" : "ppt-span-choose"
        }).text(function(a) {
            a = a.node;
            return "(" + a.internalLabel + ":`" + a.label + "`)"
        });
        a.cypherviewer.querySpanElements.filter(function(b) {
            return b.type === a.cypherviewer.QueryElementTypes.LINK
        }).attr("class", "ppt-span-link").text(function(b) {
            return "-[:`" + b.link.label + "`]-" + (a.query.USE_RELATION_DIRECTION ? ">" : "")
        });
        a.cypherviewer.querySpanElements.filter(function(b) {
    ...
    a.cypherviewer.generateData = function(b) {
        var c = [],
            d = 0,
            f = a.graph.getRootNode();
        b = a.query.getRelevantLinks(f, f, b);
        c.push({
            id: d++,
    ...
    a.query = {};
    a.query.RESULTS_PAGE_SIZE =
        100;
    a.query.VALUE_QUERY_LIMIT = 100;
    ...
    a.query.generateTaxonomyCountQuery = function(b) {
        ...
        return c === a.query.NEO4J_INTERNAL_ID ? "MATCH (n:`" + b + "`)" + (0 < d.length ? " WHERE " +
            d.join(" AND ") : "") + " RETURN count(DISTINCT ID(n)) as count" : "MATCH (n:`" + b + "`)" + (0 < d.length ? " WHERE " + d.join(" AND ") : "") + " RETURN count(DISTINCT n." + c + ") as count"
    };
    a.query.generateQueryElements = function(b, c, d, f) {
        var e = [],
            g = [],
            h = [],
            k = {},
            n = a.query.USE_RELATION_DIRECTION ? "->" : "-";
        a.provider.getPredefinedConstraints(b.label).forEach(function(a) {
            g.push(a.replace(RegExp("\\$identifier", "g"), b.internalLabel))
        });
        if (b.value && (f || b.immutable)) {
            var l = a.provider.getConstraintAttribute(b.label);
            if (l === a.query.NEO4J_INTERNAL_ID) e.push("(" +
                b.internalLabel + ":`" + b.label + "`)"), g.push("ID(" + b.internalLabel + ") = " + b.value.internalID);
            else {
                var p = b.internalLabel + "_" + l;
                k[p] = b.value.attributes[l];
                e.push("(" + b.internalLabel + ":`" + b.label + "`{`" + l + "`:{`" + p + "`}})")
            }
        } else e.push("(" + b.internalLabel + ":`" + b.label + "`)");
        var t = 0;
        d.forEach(function(d) {
            var l = d.source,
                ...
                p = "r" + t++;
            h.push(p);
            a.provider.getPredefinedConstraints(m.label).forEach(function(a) {
                g.push(a.replace(RegExp("\\$identifier", "g"), m.internalLabel))
            });
            if (m.value && m !== c && (f || b.immutable)) {
                var q =
                    a.provider.getConstraintAttribute(m.label),
                    r = m.internalLabel + "_" + q;
                k[r] = m.value.attributes[q];
                q === a.query.NEO4J_INTERNAL_ID ? (e.push("(" + l.internalLabel + ":`" + l.label + "`)-[" + p + ":`" + d.label + "`]" + n + "(" + m.internalLabel + ":`" + m.label + "`)"), g.push("ID(" + m.internalLabel + ") = " + m.value.internalID)) : e.push("(" + l.internalLabel + ":`" + l.label + "`)-[" + p + ":`" + d.label + "`]" + n + "(" + m.internalLabel + ":`" + m.label + "`{`" + q + "`:{`" + r + "`}})")
            } else e.push("(" + l.internalLabel + ":`" + l.label + "`)-[" + p + ":`" + d.label + "`]" + n + "(" + m.internalLabel +
                ":`" + m.label + "`)")
        });
        return {
            matchElements: e,
    ...
    a.query.getRelevantLinks = function(a, c, d) {
        ...
        b.forEach(function(a) {
            (a.target.value || a.target === c) && e.push(a)
        });
        ...
        e.forEach(function(c) {
            ...
            for (c = !0; c;) {
                ...
                null === e ? c = !1 : e.source === a ? (g.push(e), b.splice(b.indexOf(e), 1), c = !1) : (g.push(e), b.splice(b.indexOf(e), 1), d = e.source)
            }
        });
        return e.concat(g)
    };
    a.query.getLinksToRoot = function(b, c) {
        for (var d = [], f = b; f !== a.graph.getRootNode();) {
            ...
            e && (d.push(e), f = e.source)
        }
        return d
    };
    a.query.generateResultQuery = function(b) {
        var c = a.graph.getRootNode(),
            d = a.query.generateQueryElements(c, c, a.query.getRelevantLinks(c, c, a.graph.force.links()), !0),
            f = d.matchElements,
            e = d.whereElements,
            g = d.relationElements,
            h = [],
            k = [],
            d = d.parameters,
            n = a.provider.getResultOrderByAttribute(c.label);
        if (n) {
            var l =
                a.provider.isResultOrderAscending(c.label) ? "ASC" : "DESC";
            k.push("ORDER BY " + n + " " + l)
        }
        k.push("LIMIT " + a.query.RESULTS_PAGE_SIZE);
        n = a.provider.getReturnAttributes(c.label);
        l = a.provider.getConstraintAttribute(c.label);
        if (b) h.push(c.internalLabel), g.forEach(function(a) {
            h.push(a)
        });
        else
            for (b = 0; b < n.length; b++) g = n[b], g === a.query.NEO4J_INTERNAL_ID ? g == l ? h.push("ID(" + c.internalLabel + ") AS " + a.query.NEO4J_INTERNAL_ID.queryInternalName) : h.push("COLLECT(DISTINCT ID(" + c.internalLabel + ")) AS " + a.query.NEO4J_INTERNAL_ID.queryInternalName) :
                g == l ? h.push(c.internalLabel + "." + g + " AS " + g) : h.push("COLLECT(DISTINCT " + c.internalLabel + "." + g + ") AS " + g);
        b = "MATCH " + f.join(", ") + (0 < e.length ? " WHERE " + e.join(" AND ") : "") + " RETURN DISTINCT " + h.join(", ") + " " + k.join(" ");
        c = a.provider.filterResultQuery(c.label, {
            statement: b,
            matchElements: f,
            whereElements: e,
            returnElements: h,
    ...
    a.query.generateNodeCountQuery = function(b) {
        var c = a.query.generateQueryElements(a.graph.getRootNode(),
                b, a.query.getRelevantLinks(a.graph.getRootNode(), b, a.graph.force.links()), !0),
            d = c.matchElements,
            f = c.whereElements,
            e = [],
            c = c.parameters,
            g = a.provider.getConstraintAttribute(b.label);
        g === a.query.NEO4J_INTERNAL_ID ? e.push("count(DISTINCT ID(" + b.internalLabel + ")) as count") : e.push("count(DISTINCT " + b.internalLabel + "." + g + ") as count");
        g = "MATCH " + d.join(", ") + (0 < f.length ? " WHERE " + f.join(" AND ") : "") + " RETURN " + e.join(", ");
        b = a.provider.filterNodeCountQuery(b, {
            statement: g,
            matchElements: d,
            whereElements: f,
            returnElements: e,
    ...
    a.query.generateNodeValueQuery = function(b) {
        var c = a.graph.getRootNode(),
            ...
            h = [],
            d = d.parameters,
            k = a.provider.getValueOrderByAttribute(b.label);
        if (k) {
            var n = a.provider.isValueOrderAscending(b.label) ? "ASC" : "DESC";
            h.push("ORDER BY " + k + " " + n)
        }
        h.push("LIMIT " + a.query.VALUE_QUERY_LIMIT);
        for (var k =
                a.provider.getReturnAttributes(b.label), n = a.provider.getConstraintAttribute(b.label), l = 0; l < k.length; l++) k[l] === a.query.NEO4J_INTERNAL_ID ? k[l] == n ? g.push("ID(" + b.internalLabel + ") AS " + a.query.NEO4J_INTERNAL_ID.queryInternalName) : g.push("COLLECT (DISTINCT ID(" + b.internalLabel + ")) AS " + a.query.NEO4J_INTERNAL_ID.queryInternalName) : k[l] == n ? g.push(b.internalLabel + "." + k[l] + " AS " + k[l]) : g.push("COLLECT(DISTINCT " + b.internalLabel + "." + k[l] + ") AS " + k[l]);
        k = a.provider.getConstraintAttribute(c.label);
        k === a.query.NEO4J_INTERNAL_ID ?
            g.push("count(DISTINCT ID(" + c.internalLabel + ")) AS count") : g.push("count(DISTINCT " + c.internalLabel + "." + k + ") AS count");
        c = "MATCH " + f.join(", ") + (0 < e.length ? " WHERE " + e.join(" AND ") : "") + " RETURN DISTINCT " + g.join(", ") + " " + h.join(" ");
        b = a.provider.filterNodeValueQuery(b, {
            statement: c,
            matchElements: f,
            whereElements: e,
            returnElements: g,
    ...
    a.query.generateNodeRelationQuery = function(b) {
        ...
        g.push("ORDER BY count(r) DESC");
        var h = "MATCH " + c.join(", ") + (0 < f.length ? " WHERE " + f.join(" AND ") : "") + " RETURN " + e.join(", ") +
            " " + g.join(" ");
        b = a.provider.filterNodeRelationQuery(b, {
            statement: h,
            matchElements: c,
            whereElements: f,
            returnElements: e,
    ...
    a.result.onGraphResultReceived = function(b) {
        a.result.graphResultListeners.push(b)
    };
    a.result.parseResultData = function(b) {
        var c = [];
        if (b && b.hasOwnProperty("results") && 0 < b.results.length)
            for (var d = 0; d < b.results[0].data.length; d++) {
                for (var f = {
                        resultIndex: d,
                        label: a.graph.getRootNode().label,
                        attributes: {}
                    }, e = 0; e < b.results[0].columns.length; e++) f.attributes[b.results[0].columns[e]] = "" + b.results[0].data[d].row[e];
                c.push(f)
            }
        return c
    };
    a.result.parseGraphResultData = function(a) {
        var b = {},
            d = {};
        a.results[1].data.forEach(function(a) {
            a.graph.nodes.forEach(function(a) {
                b.hasOwnProperty(a.id) ||
                    (b[a.id] = a)
            });
            a.graph.relationships.forEach(function(a) {
                d.hasOwnProperty(a.id) || (d[a.id] = a)
            })
        });
        a = [];
        var f = [],
            e;
        for (e in b) b.hasOwnProperty(e) && a.push(b[e]);
        for (var g in d) d.hasOwnProperty(g) && f.push(d[g]);
        return {
            nodes: a,
            edges: f
        }
    ...
    a.result.updateResults = function() {        
        if (a.result.hasChanged) {
            var b = a.query.generateResultQuery(),
                b = {
                    statements: [{
                        statement: b.statement,
                        parameters: b.parameters,
            ...
            if (0 < a.result.graphResultListeners.length) {
                var c = a.query.generateResultQuery(!0);
                b.statements.push({
                    statement: c.statement,
                    parameters: c.parameters,
            ...
            a.rest.post(b).done(function(b) {
                b.errors && 0 < b.errors.length && a.logger.error("Cypher query error:" + JSON.stringify(b.errors));
                var c = a.result.parseResultData(b);
                a.result.elementos = c;
                a.result.resultListeners.forEach(function(a) {
                    a(c)
                });
                //window.alert(a.result.graphResultListeners.length);
                if (0 < a.result.graphResultListeners.length) {
                    var d = a.result.parseGraphResultData(b);
                    a.result.graphResultListeners.forEach(function(a) {
                        a(d)
                    })
                }
                //window.alert(a.result.isActive)
                a.result.isActive && (b = d3.select("#" + a.result.containerId).selectAll(".ppt-result").data([]),
                    b.exit().remove(), b = d3.select("#" + a.result.containerId).selectAll(".ppt-result").data(c, function(a) {
                        ...
                        //window.alert(a.provider.getDisplayResultFunction(b.label)(d3.select(this)))
                    }));
                    //window.alert(a.result)
    ...
    a.result.updateResultsCount = function() {
        0 < a.result.resultCountListeners.length && a.result.resultCountListeners.forEach(function(b) {
            b(a.graph.getRootNode().count)
        })
    };

    /*FUNCAO PARA EXPORTAR RESULTADO PARA CSV*/ 
    a.result.exportarResultado = function() {
        saida = [];
        row = a.result.elementos;
        d = a.provider.getReturnAttributes("Servidor");  
        //window.alert() 
        colunas = [d[0],d[1],d[2],d[3],d[4],d[5]]
        d = [];
        ...
        // INCLUINDO LINHA DE TITULO
        saida.push(d + "\r\n");
        for (var j = 0; j < row.length; j++) {            
            //d = a.provider.getReturnAttributes(row[j].label)
            temp = "";
            for (var i = 0; i < colunas.length; i++) {
                if (i == 0) {
                   //window.alert(row[j].attributes[colunas[i]]) 
                   temp = row[j].attributes[colunas[i]];      
                } else {
                   temp = temp + ";" + row[j].attributes[colunas[i]];     
                }
            }
            saida.push(temp + "\r\n");                                                
        }
        a.result.exportToCsv("saida.csv", saida) 

    };   // FIM DA FUNCAO DE EXPORTAR PARA CSV


    a.result.exportToCsv = function(filename, rows) {
        var csvFile = rows;
    ...
    a.provider.getProvider = function(b) {
        if (void 0 === b) a.logger.error("Node label is undefined, no label provider can be found.");
            ...
            if (!a.provider.nodeProviders.hasOwnProperty(b)) {
                ...
                for (var c in a.provider.nodeProviders)
                    if (a.provider.nodeProviders.hasOwnProperty(c)) {
                        ...
                        if (d.hasOwnProperty("children") && -1 < d.children.indexOf(b)) {
                            ...
                            for (var f in d) d.hasOwnProperty(f) && "children" != f && "parent" != f && (c[f] = d[f]);
                ...
                for (var e in a.provider.DEFAULT_PROVIDER) a.provider.DEFAULT_PROVIDER.hasOwnProperty(e) && (a.provider.nodeProviders[b][e] = a.provider.DEFAULT_PROVIDER[e])
            }
            return a.provider.nodeProviders[b]
        }
    };
    a.provider.getProperty = function(b, c) {
        ...
        if (!d.hasOwnProperty(c)) {
            for (var f = d, e = !1; f.hasOwnProperty("parent") && !e;) f = a.provider.getProvider(f.parent), f.hasOwnProperty(c) && (d[c] = f[c], e = !0);
            e || (a.logger.debug('No "' + c + '" property found for node label provider (' +
                b + "), default value will be used"), a.provider.DEFAULT_PROVIDER.hasOwnProperty(c) ? d[c] = a.provider.DEFAULT_PROVIDER[c] : a.logger.debug('No default value for "' + c + '" property found for label provider (' + b + ")"))
        }
        return d[c]
    };
    ...
    a.provider.getIsAutoExpandRelations = function(b) {
        return a.provider.getProperty(b, "autoExpandRelations")
    };
    a.provider.getSchema = function(b) {
        return a.provider.getProperty(b, "schema")
    };
    a.provider.getReturnAttributes =
        function(b) {
            var c = a.provider.getProvider(b),
                d = {};
            if (c.hasOwnProperty("returnAttributes"))
                for (var f = 0; f < c.returnAttributes.length; f++) c.returnAttributes[f] === a.query.NEO4J_INTERNAL_ID ? d[a.query.NEO4J_INTERNAL_ID.queryInternalName] = !0 : d[c.returnAttributes[f]] = !0;
            for (; c.hasOwnProperty("parent");)
                if (c = a.provider.getProvider(c.parent), c.hasOwnProperty("returnAttributes"))
                    for (f = 0; f < c.returnAttributes.length; f++) c.returnAttributes[f] === a.query.NEO4J_INTERNAL_ID ? d[a.query.NEO4J_INTERNAL_ID.queryInternalName] = !0 : d[c.returnAttributes[f]] = !0;
            ...
            b = [];
            for (var e in d) d.hasOwnProperty(e) && (e == a.query.NEO4J_INTERNAL_ID.queryInternalName ?
                b.push(a.query.NEO4J_INTERNAL_ID) : b.push(e));
    ...
    a.provider.filterResultQuery = function(b, c) {
        return a.provider.getProperty(b, "filterResultQuery")(c)
    };
    ...
    a.provider.filterNodeValueQuery = function(b, c) {
        return a.provider.getProperty(b.label, "filterNodeValueQuery")(b, c)
    };
    a.provider.filterNodeCountQuery = function(b, c) {
        return a.provider.getProperty(b.label, "filterNodeCountQuery")(b, c)
    };
    a.provider.filterNodeRelationQuery =
        function(b, c) {
            return a.provider.getProperty(b.label, "filterNodeRelationQuery")(b, c)
        };
    a.provider.getDisplayResultFunction = function(b) {
        return a.provider.getProperty(b, "displayResults")
    };
    a.provider.DEFAULT_PROVIDER = {
        isSearchable: !0,
        ...
        getSemanticValue: function(b) {
         ...
         d = a.provider.getReturnAttributes(b.label)
            
        var c = b.label
            
        var c = a.provider.getProperty(b.label, "constraintAttribute");        
        if (d.length < 3) {
        retorno = b.type === a.graph.node.NodeTypes.VALUE ? 
                c === a.query.NEO4J_INTERNAL_ID ? 
                    "" + b.internalID : "" + b.attributes[c] : void 0 === b.value ? b.label : c === a.query.NEO4J_INTERNAL_ID ? "" + b.value.internalID : "" + b.value.attributes[c]
        } else {
                    retorno = b.type === a.graph.node.NodeTypes.VALUE ? 
                c === a.query.NEO4J_INTERNAL_ID ? 
                    "" + b.internalID : "" + b.attributes[c] : void 0 === b.value ? b.label : c === a.query.NEO4J_INTERNAL_ID ? "" + b.value.internalID : "" + b.value.attributes["nome"]

//            retorno = b.value.attributes["nome"]
        }
        

        return retorno        
        //para mudar o parametro da consulta
        },
        ...
        filterNodeValueQuery: function(a, c) {
            return c
        },
        filterNodeCountQuery: function(a, c) {
            return c
        },
        filterNodeRelationQuery: function(a, c) {
            return c
        },
        displayResults: function(b) {
            //var elementos = [];
            //window.alert(b.data()[0])
            var c = b.data()[0],
                // local para incluir os 
                d = a.provider.getReturnAttributes(c.label),                
                f = b.append("table").attr("class", "ppt-result-table");
                //d = d.splice(d.indexOf(a.provider.getProperty(b.label, "constraintAttribute")),1);
                //window.alert(c.label)
                if (d.length > 3) {
                    temp = [d[0],d[1],d[2],d[3],d[4],d[5]]
                    d = temp
                    temp = ""
                }                                
            d.forEach(function(b) {
                var d = b;
                a.query.NEO4J_INTERNAL_ID ===
                    b && (d = a.query.NEO4J_INTERNAL_ID.queryInternalName);
                var e = f.append("tr");
                e.append("th").text(function() {
                    return b === a.query.NEO4J_INTERNAL_ID ? "internal ID:" : b + ": "
                });                
                void 0 !== c.attributes[d] && e.append("td").text(function(a) {

                    //window.alert(a.attributes[d])
                    //return a.attributes[d]
                    return a.attributes[d].replace(","," | ");
                })                
            })
        }
    };
